...
export default function OrderDetailPage() {
  ...
  const convertNumberToWords = (num: number): string => {
    ...
    if (num < 1000) {
      ...
      const rest = num % 100;
      return ones[hundred] + ' trăm' + (rest ? ' ' + convertNumberToWords(rest) : '');
    }
    if (num < 1000000) {
      const thousand = Math.floor(num / 1000);
      const rest = num % 1000;
      return convertNumberToWords(thousand) + ' nghìn' + (rest ? ' ' + convertNumberToWords(rest) : '');
    }
    
    const million = Math.floor(num / 1000000);
    const rest = num % 1000000;
    return convertNumberToWords(million) + ' triệu' + (rest ? ' ' + convertNumberToWords(rest) : '');
  };
  ...
  const handleExportPDF = () => {
    ...
    const totalInWords = convertNumberToWords(Math.floor(order.total_amount / 1000));
        ...
        <body>
          <div class="header">
            ...
            <div style="text-align: right;">
              <div><strong>Ngày:</strong> ${new Date(order.created_at).toLocaleDateString('vi-VN')}</div>
    ...
    <main className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <div className="text-center mb-6">
          <h1 className="text-3xl font-bold text-gray-900">ĐƠN HÀNG SẢN XUẤT</h1>
          <p className="text-lg text-gray-600 mt-2">Mã đơn: {order.order_code}</p>
        </div>
          ...
          <div>
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex">
                ...
                <span className="text-gray-900">{new Date(order.created_at).toLocaleDateString('vi-VN')}</span>
              </div>
              <div className="flex">
        ...
        <h3 className="font-semibold text-gray-900 mb-4 text-lg">Chi tiết sản phẩm</h3>
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {items.map((item, index) => (
              <tr key={item.id}>
                ...
                <td className="px-4 py-3 text-sm text-gray-900 text-right">{item.price.toLocaleString()}đ</td>
          ...
          <tfoot className="bg-gray-50">
            ...
            {order.debt_amount > 0 && (
              <tr>
                <td colSpan={5} className="px-4 py-3 text-right font-bold text-red-600">
                  Công nợ:
                </td>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <h3 className="font-semibold text-gray-900 mb-4 text-lg">Tiến độ sản xuất</h3>
          ...
          {processes.map((process, index) => (
            <div key={process.id} className="flex items-center">
              ...
              <div className="ml-4 flex-1">
                <div className="flex justify-between items-center">
                  <div>
                    <span className="font-semibold text-gray-900">{process.process_name}</span>
                    <span className="ml-2 text-sm text-gray-500">({process.process_code})</span>
                  </div>
                  {getStatusBadge(process.status)}
                </div>
                {process.start_time && (
                  <div className="text-sm text-gray-600 mt-1">
                    Bắt đầu: {new Date(process.start_time).toLocaleString('vi-VN')}
                    {process.end_time && ` - Kết thúc: ${new Date(process.end_time).toLocaleString('vi-VN')}`}
                  </div>
                )}
              </div>
